...
import { useGetSingleCategoryBySlugQuery, useUpdateCategoryMutation } from "@/redux/features/category/categoryApi";
...
const UpdatePage: React.FC<{ params: Promise<ParamsSlug> }> = ({ params }) => {
  ...
  const { data, refetch } = useGetSingleCategoryBySlugQuery(slug);
  useEffect(() => {
  ...
  return (
    <div>
      <Heading
        ...
        subTitle="Edit the details of an existing Category."
      />
      <Form {...methods}>
        <form
