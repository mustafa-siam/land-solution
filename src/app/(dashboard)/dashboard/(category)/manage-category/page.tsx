...
import { useDeleteCategoryMutation, useGetAllCategoriesQuery, useUpdateCategoryStatusMutation, useUpdateCategoryTrashStatusMutation } from "@/redux/features/category/categoryApi";

// Using standard Radix / shadcn Select components for inline data status modifications
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ManagePage() {
  ...
  const { data, isLoading, isError, isFetching } = useGetAllCategoriesQuery(queryParams);
  const [deleteMethod, { isLoading: isDeleting }] = useDeleteCategoryMutation();
  const [updateTrash, { isLoading: isUpdatingTrash }] = useUpdateCategoryTrashStatusMutation();
  ...
  return (
    <div className="space-y-6">
      <GlobalHeaderSection
        ...
        description="View, edit, and manage all Categories in the system"
        meta={meta}
                          ...
                          {filters?.isTrash || (
                            <>
                              <DropdownMenuItem className="cursor-pointer" asChild>
                                <Link
                                  href={`/dashboard/update-category/${item.slug}`}
