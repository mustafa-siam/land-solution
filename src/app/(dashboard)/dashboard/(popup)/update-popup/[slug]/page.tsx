...
import { useGetSinglePopupBySlugQuery, useUpdatePopupMutation } from "@/redux/features/popup/popupApi";
...
const UpdatePage: React.FC<{ params: Promise<ParamsSlug> }> = ({ params }) => {
  ...
  const { data, refetch } = useGetSinglePopupBySlugQuery(slug);
  useEffect(() => {
...
//   Object.entries(data).forEach(([key, value]) =>
    ...
    formData.append("link",metaData?.link); // send as JSON string
    ...
    refetch();
    router.push("/dashboard/manage-popup");
  } catch (error: any) {
    console.error({ error });
    toast.error(
  ...
  return (
    <div>
      <Heading
        title="Update Popup"
        subTitle="Edit the details of an existing Popup."
      />
      <Form {...methods}>
        <form
        ...
        >
          ...
          >
            {isLoading ? "Please Wait..." : "Update Popup"}
          </Button>
