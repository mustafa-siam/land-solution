...
import { useDeleteAreaMutation, useGetAllAreasQuery, useUpdateAreaStatusMutation, useUpdateAreaTrashStatusMutation } from "@/redux/features/area/areaApi";

// Shadcn / Radix Select UI Components for inline table changes
import {
  Select,
  SelectContent,
  SelectItem,
...
export default function ManagePage() {
  ...
  const { data, isLoading, isError, isFetching } = useGetAllAreasQuery(queryParams);
  const [deleteMethod, { isLoading: isDeleting }] = useDeleteAreaMutation();
  const [updateTrash, { isLoading: isUpdatingTrash }] = useUpdateAreaTrashStatusMutation();
  const [updateStatus] = useUpdateAreaStatusMutation();
  ...
  const handleInlineStatusChange = async (id: string, newStatus: string) => {
    try {
      // Flat payload execution without nesting under data property
  ...
  return (
    <div className="space-y-6">
      <GlobalHeaderSection
        name="Manage Area"
        description="View, edit, and manage all Areas in the system"
        meta={meta}
                          ...
                          {filters?.isTrash || (
                            <>
                              <DropdownMenuItem className="cursor-pointer" asChild>
                                <Link
                                  href={`/dashboard/update-area/${item.slug}`}
