...
import { useGetSingleProductBySlugQuery, useUpdateProductMutation } from "@/redux/features/product/productApi";
import {
  Select,
  SelectContent,
  SelectGroup,
...
const UpdatePage: React.FC<{ params: Promise<ParamsSlug> }> = ({ params }) => {
  ...
  const [handleUpdate, { isLoading, error }] =
    useUpdateProductMutation();
      ...
      const allData : any[] = useMemo(() => categories?.data?.data || [], [categories]);
  ...
  const { data, refetch } = useGetSingleProductBySlugQuery(slug);
  useEffect(() => {
...
//   Object.entries(data).forEach(([key, value]) =>
    ...
    if (metaData.image) {
      ...
      metaData.image.file.forEach((file: File) => {
        formData.append("file", file); // backend must handle multiple files if needed
      });
    }
...
metaData.metaSeoTags.forEach((item: string) => {
  ...
  if (data?.data) {
    ...
    newData.areaId = payload?.areaId?._id;
    newData.categoryId = payload?.categoryId?._id;
    newData.gas = payload.gas;
    newData.facing = payload.facing;
    newData.electricity = payload.electricity;
    newData.water = payload.water;
    newData.availability = payload.availability;
    newData.propertyType = payload.propertyType;
    setMetaData(newData);
    setMetaDataHistory(newData);
    methods.reset(newData);
    setId(payload._id);
  ...
  return (
    <div>
      <Heading
        ...
        subTitle="Edit the details of an existing Product."
      />
       <Form {...methods}>
              <form
